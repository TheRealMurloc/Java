...
public class FloristsTest {
  ...
  static int valueOf(Box box, String color) {
      int tmp = 0;
      for(int i = 0; i < Box.pudelko.size(); i++)
      {
    	  if(Box.pudelko.get(i).kolor.equals(color))
    	  {
    		  tmp += (Box.pudelko.get(i).ilosc) * (PriceList.getInstance().hm.get(Box.pudelko.get(i).nazwa));
    	  }
      }
      return tmp;
  }
